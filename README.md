...
Design Patterns: 
* Composite: 
  * In an rpg, most objects and characters follow the same formula but are still of different types and have different attributes. Without using a composite pattern, there is no way to have multiple attributes added to a character without having a lot of member data and an unrealistic amount of conditional statements if attributes are dependent on others or nested. With a composite pattern, you can recursively compose attributes inside of attributes and greatly reduce the need for if-else statements and member data. For instance, different attributes will most likely derive from an abstract base class Attribute. So when it comes to implementing new attributes, rather than keep track of every new attribute to make sure they can add new associated ones, it will be much easier to have that attribute derive from the abstract class. Therefore, the use of a composite design pattern allows for a more streamlined implementation of nested attributes and having branches of them. 
 * Decorator: 
